<template>
  <div class="container-item">
        <div class="container-avatarAndText">
          <div class="author">
            <div class="avatar">
              <div class="avatarInner"><img :src="data.user.avatar">
                <svg class="icon" aria-hidden="true" v-if="data.user.is_expert === 1">
                  <use xlink:href="#icon-zhuanjiabiaojishixin"></use>
                </svg>
              </div>
            </div>
            <div class="mui-media-body">{{data.title}}</div>
          </div>
        </div>
        <div class="text-16-444 mui-ellipsis-2"> {{data.feed.title}}</div>
        <div class="labelWrapper"><span class="font-12 color-b4b4b6 line-vertical">回答 {{data.feed.answer_num}}</span><span class="font-12 color-03aef9">关注问题 {{data.feed.follow_num}}</span></div>
      </div>
</template>


<script type="text/javascript">
  export default {
    data () {
      return {

      }
    },
    components: {

    },
    props: {
      data: {
        type: Object,
        default: {}
      },
    },
    created(){
    },
    watch: {
    },
    mounted(){
      
    },
    methods: {

    }
  };
</script>
