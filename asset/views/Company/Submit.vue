<template>
  <div>
      ...
      <ul class="mui-table-view companyForm noBottomBorder">
        ...
        <li :class="{'mui-table-view-cell':true, noBottomBorder:industryTags.length}">
          ...
          <div class="selectedWrapper">
                    ...
                    <span class="selected" v-for="(industry, index) in industryTags">{{industry.text}}<svg class="icon"
                                                                                                           aria-hidden="true"
                                                                                                           @tap.stop.prevent="closeIndustry(index)">
        ...
        <li class="mui-table-view-cell">
          <div class="mui-input-row">
            ...
            <input type="text" @tap.stop.prevent="selectCompanyWorkers()" pattern="\d*" v-model="company_workers.text"
                   name="company_workers_text" readonly="readonly" v-if="company_workers.text">

            <input type="text" @tap.stop.prevent="selectCompanyWorkers()" pattern="\d*" v-model="company_workers.text"
                   name="company_workers_text" readonly="readonly" placeholder="选择公司人数" v-else>
          </div>
        </li>
        <li class="mui-table-view-cell">
          <div class="mui-input-row">
      ...
      <ul class="mui-table-view companyForm">
        <li class="mui-table-view-cell">
          <div class="mui-input-row">
            ...
            <div class="textRight">
              ...
              <span class="mui-radio radioWrapper">
                    <input name="radio1" type="radio" checked="checked" v-model="company_represent_person_is_self"
                           value="0">
                    其他人
                </span>
            </div>
        ...
        <li class="mui-table-view-cell">
          <div class="mui-input-row">
            ...
            <input type="text" name="company_represent_person_name_1" v-model="company_represent_person_name"
        ...
        <li class="mui-table-view-cell">
          <div class="mui-input-row">
            ...
            <input type="text" name="company_represent_person_title_1" v-model="company_represent_person_title"
                   readonly="readonly" v-else>
          </div>
        </li>
        <li class="mui-table-view-cell">
          <div class="mui-input-row">
            ...
            <input type="text" name="company_represent_person_phone_1" pattern="\d*"
                   v-model="company_represent_person_phone" readonly="readonly" v-else>
          </div>
        </li>
        <li class="mui-table-view-cell">
          <div class="mui-input-row">
            ...
            <input type="text" name="company_represent_person_email1" v-model="company_represent_person_email"
                   readonly="readonly" v-else>
          </div>
        </li>
      </ul>
    ...
    <div id="page_industry_tags" class="mui-modal mui-pageSub">
      ...
      <industry-tags-indexed-list :tag_type="3" :selected="industryTags" :back_id="page_industry_tags_id"
  ...
  export default {
    ...
    methods: {
      ...
      alertProtocol () {
        ...
        window.mui.alert(html, '选择验证模式', '确定', () => {
          return
        }, 'div')
      },
      closePopup () {
        window.mui('body').on('tap', '.mui-popup-backdrop', () => {
          window.mui.closePopup()
        })

        window.mui('body').on('tap', '.mui-popup-button', () => {
          window.mui.closePopup()
          this.submitLast()
        })
      },
      ...
      submit () {
        ...
        if (this.industryTags.length < 1) {
          window.mui.toast('请输入行业领域')
          return
        }

        if (!this.company_workers.value) {
          window.mui.toast('请输入公司规模')
          return
        }

        if (!this.company_credit_code) {
          window.mui.toast('请输入统一社会信用代码')
          return
        }
        ...
        if (!this.company_represent_person_title) {
          window.mui.toast('请输入对接人职位')
          return
        }

        if (!this.company_represent_person_phone) {
          window.mui.toast('请输入对接人手机')
          return
        }
