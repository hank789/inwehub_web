				...
				<div class="cardWrapper">
					<div class="card">
						...
						<div class="detail">
							...
							<!--文章和评论-->
							<div class="news">
								<p class="mui-ellipsis" @tap.stop.prevent="$router.pushReadHubPage('/@'+resume.info.id)">文章<span>{{ resume.info.submission_count }}</span>篇</p>
								<i></i>
								<p class="mui-ellipsis" @tap.stop.prevent="$router.pushReadHubPage('/@'+resume.info.id)">评论<span>{{ resume.info.comment_count }}</span>条</p>
							</div>
							<!---->
	...
	export default {
		...
		methods: {
			...
			bindWechatShare() {
				...
				if(mui.os.wechat) {
					//微信分享
					postRequest(`share/wechat/jssdk`, {
